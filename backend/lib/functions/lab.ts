    ...
    for (let i = 0; i < lab.courses.length; i++) {
            ...
            for (let i = 0; i < scoreValue.length; i++) {
              for (let j = 0; j < scoreValue[i].length; j++) {
                if (scoreValue[i][j] < 0) {
                  ...
                  if (score[i][j] >= 0) score[i][j] += scoreValue[i][j];
                }
              }
            }
          }
      ...
      if (!timetable) {
        timetable = Array(score.length)
          .fill(null)
          .map(() => Array(score[0].length).fill("0"));
      }
      ...
      if (maxSumIndices.i !== -1 && maxSumIndices.j !== -1) {
        ...
        let score: number[][] = Array(6)
          .fill(0)
          .map(() => Array(6).fill(0));
        for (let t = 0; t < teachers.length; t++) {
          let teacherScore = scoreTeachers(
            teachers[t].timetable,
            teachers[t].labtable
          );
        ...
        for (let day = 0; day < score.length && !alloted; day++) {
          for (
            let period = 0;
            period < score[day].length && !alloted;
            period += 2
          ) {
            if (
              score[day][period] >= 0 &&
              ...
              if (courseIndex !== -1) {
                ...
                for (let t = 0; t < subTeachers.length; t++) {
                  let { status, teacher } = await peekTeacher(
                    token,
                    subTeachers[t]
                  );
                  if (status == statusCodes.OK && teacher) {
...
export async function recommendLab(
  ...
  try {
    ...
    return {
      status: statusCodes.OK,
      timetable: convertTableToString(
        score.map((row) => row.map((val) => val.toString()))
      ),
    };
  } catch {
    return {
