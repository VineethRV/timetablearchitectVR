...
async function createLab(JWTtoken, name, semester, batches, teachers, rooms, timetables, department) {
    try {
...
async function deleteLabs(JWTtoken, labs) {
    try {
        const { status, user } = await auth.getPosition(JWTtoken);
        if ((user === null || user === void 0 ? void 0 : user.orgId) == null) {
...
async function getLabs(JWTtoken, department = null, semester = null) {
    try {
        const { status, user } = await auth.getPosition(JWTtoken);
        if ((user === null || user === void 0 ? void 0 : user.orgId) == null) {
            return {
...
async function updateLab(JWTtoken, originalName, originalSemester, lab, originalDepartment) {
    try {
        const { status, user } = await auth.getPosition(JWTtoken);
        if ((user === null || user === void 0 ? void 0 : user.orgId) == null) {
            return {
...
async function peekLab(JWTtoken, name, semester, department) {
    try {
        const { status, user } = await auth.getPosition(JWTtoken);
        if ((user === null || user === void 0 ? void 0 : user.orgId) == null) {
            return {
